...
def main(in_path, out_path, key):
    ...
    for night, grp in patterns.groupby("night"):
        ...
        stage_command = 'ssh coma head -c 1 {}'.format(' '.join(list(grp['path'].values)))
        print(stage_command)
        ...
        rsync_drs_command = 'rsync -av --stats --progress coma:{} {}'.format(
        ...
        for pattern in grp['path']:
            rsync_command = 'rsync -av --stats --progress coma:{} {}'.format(pattern, outputpath)
            print(rsync_command)
